...
@receiver(pre_save, sender='documents.Document')
def store_old_document_status(sender, instance, **kwargs):
    """Store old status before document is saved."""
    if instance.pk:
        try:
            old_instance = sender.objects.get(pk=instance.pk)
            instance._old_doc_status = old_instance.status
        except sender.DoesNotExist:
            instance._old_doc_status = None
    else:
        instance._old_doc_status = None
...
def create_document_notification(sender, instance, created, **kwargs):
    """
    Create notification when document status changes to verified/rejected.
    
    Notifies: Document owner
    """
    if created:
        return  # Don't notify on upload
    
    document = instance
    old_status = getattr(instance, '_old_doc_status', None)
    new_status = document.status
    
    # Only notify on status change to verified or rejected
    if old_status == new_status:
        return
    
    if new_status not in ('verified', 'rejected', 'not_allowed'):
        return
    ...
    if new_status == 'verified':
        ...
        message = f"Документ «{document.name}» успешно проверен"
    else:
        ...
        message = f"Документ «{document.name}» отклонён: {reason}"
    ...
    # Add application_id if document is attached to applications
    if hasattr(document, 'applications') and document.applications.exists():
        data['application_id'] = document.applications.first().id
    ...
    if document.owner:
        try:
            ...
            logger.info(f"Created document notification for user {document.owner.id}")
        except Exception as e:
            logger.error(f"Failed to create document notification: {e}")
