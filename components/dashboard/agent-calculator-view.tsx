...
const LAWS = ["44-ФЗ", "223-ФЗ", "185-ФЗ (615-ПП)", "275-ФЗ", "КБГ (Коммерческие)"]
...
const LEASING_BANKS: BankOffer[] = [
    { name: "Эволюция (Лизинг)", minAmount: 500000, maxAmount: 500000000, bgRate: 0, creditRate: 0, speed: "Высокая", laws: [], type: "leasing" },
    { name: "Carcade (Лизинг)", minAmount: 500000, maxAmount: 500000000, bgRate: 0, creditRate: 0, speed: "Высокая", laws: [], type: "leasing" },
    { name: "Европлан (Лизинг)", minAmount: 500000, maxAmount: 500000000, bgRate: 0, creditRate: 0, speed: "Высокая", laws: [], type: "leasing" },
    { name: "ВТБ Лизинг", minAmount: 1000000, maxAmount: 1000000000, bgRate: 0, creditRate: 0, speed: "Средняя", laws: [], type: "leasing" },
    { name: "ПСБ Лизинг", minAmount: 1000000, maxAmount: 1000000000, bgRate: 0, creditRate: 0, speed: "Средняя", laws: [], type: "leasing" },
    { name: "Газпромбанк Автолизинг", minAmount: 1000000, maxAmount: 1000000000, bgRate: 0, creditRate: 0, speed: "Средняя", laws: [], type: "leasing" },
    ...
    { name: "Индивидуальное рассмотрение (Лизинг)", minAmount: 10000, maxAmount: MAX_AMOUNT_FALLBACK, bgRate: 0, creditRate: 0, speed: "Высокая", laws: [], individual: true, type: "leasing" },
]
...
const calculateOffers = (
    ...
    pool.forEach((bank) => {
        ...
        } else if (productType !== "leasing" && bank.laws.length > 0 && !bank.laws.includes(law)) { // Skip law check for leasing
...
interface AgentCalculatorViewProps {
    prefill?: CalculatorPrefill | null
    onPrefillApplied?: () => void
}

export function AgentCalculatorView({ prefill, onPrefillApplied }: AgentCalculatorViewProps) {
    ...
    useEffect(() => {
        ...
        setRkoSessionId(null)
        setSpecSessionId(null)
    }, [selectedClientId])
    ...
    // Documents hook - load documents for selected client
    ...
    const matchDocumentsToRequired = () => {
        ...
        // Get ALL documents for selected client (including agent's docs where company=null)
        const allDocs = selectedClientId 
            ? companyDocuments.filter(d => 
                d.company === parseInt(selectedClientId) || 
                d.company == null
              )
            : companyDocuments
    ...
    useEffect(() => {
        ...
        if (application.company) {
            setSelectedClientId(String(application.company))
        }
    ...
    const handleCreateApplication = async () => {
        ...
        // Auto-match documents from client's library
    ...
    const getRkoDocumentIds = (context: "session" | "application") => {
        const matched = matchDocumentsToRequired()
        if (matched.missingRequired.length > 0) {
            toast.warning(`Недостающие документы: ${matched.missingRequired.map(d => d.name).join(', ')}`, {
                description: context === "session"
                    ? 'Подбор будет создан. Загрузите документы позже в карточке заявки.'
                    : 'Заявка будет создана. Загрузите документы позже в карточке заявки.'
            })
        }
    ...
    const ensureRkoSession = async (
        ...
        if (existingId) {
            if (options.navigate) {
                navigateToCalculationSession(existingId)
            }
            return existingId
        }
    ...
    const createRkoSession = async (type: "rko" | "specaccount") => {
        ...
        const documentIds = getRkoDocumentIds("session")
        await ensureRkoSession(type, documentIds, { navigate: true })
    }
    ...
    const createRkoApplication = async (bank: string, type: "rko" | "specaccount") => {
        ...
        const documentIds = getRkoDocumentIds("application")
                            ...
                            <div className="flex flex-col gap-2 pt-4 border-t border-[#2a3a5c]">
                                ...
                                <div className="flex gap-4">
                                    <Button
                                        ...
                                        className="bg-[#3CE8D1] text-[#0a1628] hover:bg-[#2fd4c0] px-8 disabled:bg-[#1f3556] disabled:text-[#b7c7e6] disabled:opacity-100 disabled:cursor-not-allowed"
                                    >
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            value={dateFrom}
                                            onChange={e => setDateFrom(e.target.value)}
                                            className="h-11 max-w-[180px] bg-[#0f1d32]/50 border-[#2a3a5c]/30 focus:border-[#3CE8D1]/50 text-white [color-scheme:dark] [&::-webkit-calendar-picker-indicator]:brightness-0 [&::-webkit-calendar-picker-indicator]:invert"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-sm text-[#94a3b8]">по</Label>
                            ...
                            <div className="pt-6 border-t border-[#2a3a5c]/30">
                                <div className="flex items-center gap-4">
                                    <Button
                                        ...
                                        className="h-12 px-8 border-2 border-[#3CE8D1] bg-transparent text-[#3CE8D1] font-semibold hover:bg-[#3CE8D1] hover:text-[#0a1628] transition-all disabled:bg-[#1f3556] disabled:text-[#b7c7e6] disabled:opacity-100 disabled:cursor-not-allowed"
                                    >
